...
/* ---- Demonstra��o ---- */
int main(void) {
    ArvoreB *T = ab_criar(3); // t=3 (m�x 5 chaves por n�)

    int inserir[] = {50, 9, 30, 60, 80, 1, 2, 3, 4, 15, 20, 40, 46, 51, 52, 56, 58, 65, 70, 85, 90, 57, 71, 72, 73};
    int ni = (int)(sizeof(inserir)/sizeof(inserir[0]));

    printf("Inserindo:\n");
    for (int i = 0; i < ni; ++i) {
        printf(" + %d\n", inserir[i]);
        ab_inserir(T, inserir[i]);
    }
    ...
    // Remo��es para exercitar casos
    int remover[] = {3, 40, 65, 50, 9, 30, 56, 58, 52, 51, 46, 20, 15, 4, 2, 1};
    int nr = (int)(sizeof(remover)/sizeof(remover[0]));
    for (int i = 0; i < nr; ++i) {
        printf("\nRemovendo %d...\n", remover[i]);
        ab_remover(T, remover[i]);
        ab_em_ordem(T);
    }
