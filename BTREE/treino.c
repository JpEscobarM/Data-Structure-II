...
/**
 * @brief Reequilibra o filho src->filhos[index] após remoção.
 * Tenta emprestar de irmãos (≥ ORDEM+1) ou fundir com vizinho para manter ≥ ORDEM chaves.
 * @param src   Nó pai.
 * @param index Índice do filho a completar.
 */
...
/**
 * @brief Remove a chave de um nó interno em src->chaves[index].
 * Usa predecessor (filho esquerdo) ou sucessor (filho direito) se puder emprestar (≥ ORDEM+1);
 * senão, funde os filhos adjacentes e remove no nó resultante, preservando as invariantes.
 * @param src   Nó interno (pai).
 * @param index Índice da chave a remover em src.
 */
void removerEmInterno(No *src, int index)
{
    if(src == NULL)
...
/**
 * @brief Remove a chave em uma Árvore B iniciando no nó src.
 * Busca a posição; se a chave estiver no nó, remove em folha ou interno.
 * Caso contrário, desce ao filho correto, reequilibrando com completarFilho()
 * quando o filho tem exatamente ORDEM chaves.
 * @param arv   Árvore (usada para ajustes de raiz, se necessário).
 * @param src   Nó onde a busca/remoção começa.
...
/**
 * @brief Remove a chave da Árvore B e ajusta a raiz se necessário.
 * Chama removerChaveNo(); se a raiz ficar com 0 chaves, zera a árvore (se folha)
